    ...
    <div className="space-y-8">
      ...
      <div className="flex gap-3">
        ...
        >
          ...
          {activeCount > 0 && (
            <span className="w-4 h-4 flex items-center justify-center rounded-full bg-background/25 text-xs font-bold">
              {activeCount}
            </span>
          )}
        </motion.button>
            ...
            <div className="p-5 rounded-xl border border-foreground/10 bg-background space-y-5">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {[
                  ...
                  <div key={key} className="space-y-2.5">
                    <p className="text-xs tracking-[0.2em] uppercase font-montserrat font-medium text-foreground/50">
                      {label}
                    </p>
                    <div className="flex flex-wrap gap-2">
                      {options.map((o) => (
                        <button
                          ...
                          className={`px-3 py-1.5 rounded-lg text-xs font-montserrat font-medium transition-all duration-200 ${filters[key] === o ? "bg-foreground text-background" : "border border-foreground/15 text-foreground hover:bg-foreground/5"}`}
                        >
                          {o}
                        </button>
                      ))}
              ...
              {activeCount > 0 && (
                <button
                  ...
                  className="text-xs font-montserrat text-foreground/50 hover:text-foreground transition-colors underline underline-offset-2"
                >
                  Clear all filters
                </button>
              )}
      ...
      {isLoading ? (
        <div className="flex justify-center items-center py-24">
          <Loader2 className="w-7 h-7 animate-spin text-foreground/40" />
        </div>
      ) : products.length === 0 ? (
